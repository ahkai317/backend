...
def feature(df):
    ...
    df["adx"] = talib.ADX(df["High"], df["Low"],
                          df["Close"], timeperiod=14)  # 9
    df["adxr"] = talib.ADXR(df["High"], df["Low"],
                            df["Close"], timeperiod=14)  # 5
    df["dx"] = talib.DX(df["High"], df["Low"],
                        df["Close"], timeperiod=14)  # 15
    df["mfi"] = talib.MFI(df["High"], df["Low"], df["Close"],
                          df["Volume"], timeperiod=14)  # 14
    df["minus_di"] = talib.MINUS_DI(
        df["High"], df["Low"], df["Close"], timeperiod=14)  # 12
    df["plus_di"] = talib.PLUS_DI(
        df["High"], df["Low"], df["Close"], timeperiod=14)  # 13
    ...
    df["adosc"] = talib.ADOSC(df["High"], df["Low"],
                              df["Close"], df["Volume"])  # 8
    df["obv"] = talib.OBV(df["Close"], df["Volume"])  # 1
    df["ht_dcperiod"] = talib.HT_DCPERIOD(df["Close"])  # 4
    df["natr"] = talib.NATR(df["High"], df["Low"],
                            df["Close"], timeperiod=14)  # 7
...
def label(df):
    def triple_barrier(price=df["Close"], ub=1.07, lb=0.97, max_period=20):
        ...
        p = price.rolling(max_period).apply(
            end_price, raw=True).shift(-max_period+1)
