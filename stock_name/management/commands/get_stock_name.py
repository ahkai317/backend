...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stock_list = pd.concat(lst).sort_values(by="c", ignore_index=True).drop(columns=["type1", "type2"])
        stock_list = stock_list.to_dict('records')
        # ================== Start to sql ==============================
        for stock in stock_list:
            StockName.objects.update_or_create(stock=stock["c"], stockName=stock["name"],
                                               industry=stock["type"])
